    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          ...
          <li>✓ Controle completo de Registro Ponto</li>
          <li>✓ Controle completo de Afastamento</li>
          <li>✓ Controle completo de Carga Horaria</li>
        </ul>
      </div>
...
function PainelFuncionario({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "cargo", "departamento", "dataAdmissao", "salario", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "cargo", "departamento", "dataAdmissao", "salario"].forEach(k => { f[k] = item[k] ?? ""; });
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>departamento</b><span>{item["departamento"] ?? "—"}</span></div>
...
function PainelRegistroPonto({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["funcionarioId", "dataHora", "tipo", "latitude", "longitude", "dispositivo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/registropontos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/registropontos/" + editId : apiBaseUrl + "/api/registropontos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["funcionarioId", "dataHora", "tipo", "latitude", "longitude", "dispositivo"].forEach(k => { f[k] = item[k] ?? ""; });
    ["funcionario"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/registropontos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["tipo"] ?? "RegistroPonto")}</div>
            ...
            <div className="item-field"><b>latitude</b><span>{item["latitude"] != null ? Number(item["latitude"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>longitude</b><span>{item["longitude"] != null ? Number(item["longitude"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>dispositivo</b><span>{item["dispositivo"] ?? "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} RegistroPonto</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
      ...
      <input type="number" step="0.01" value={form["latitude"] ?? ""} onChange={e => setForm({...form, latitude: e.target.value})} />
      <label className="field-label">longitude</label>
      <input type="number" step="0.01" value={form["longitude"] ?? ""} onChange={e => setForm({...form, longitude: e.target.value})} />
      <label className="field-label">dispositivo</label>
      <input type="text" value={form["dispositivo"] ?? ""} onChange={e => setForm({...form, dispositivo: e.target.value})} />
...
function PainelAfastamento({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["funcionarioId", "dataInicio", "dataFim", "motivo", "status", "observacoes", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/afastamentos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/afastamentos/" + editId : apiBaseUrl + "/api/afastamentos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["funcionarioId", "dataInicio", "dataFim", "motivo", "status", "observacoes"].forEach(k => { f[k] = item[k] ?? ""; });
    ["funcionario"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/afastamentos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["motivo"] ?? "Afastamento")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/afastamentos/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["motivo"] ?? "Afastamento")}</div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Afastamento</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">funcionarioId</label>
      <input type="number" value={form["funcionarioId"] ?? ""} onChange={e => setForm({...form, funcionarioId: e.target.value})} />
...
function PainelCargaHoraria({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["funcionarioId", "diasSemana", "horarioEntrada", "horarioSaida", "cargaDiaria", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/cargahorarias", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/cargahorarias/" + editId : apiBaseUrl + "/api/cargahorarias";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["funcionarioId", "diasSemana", "horarioEntrada", "horarioSaida", "cargaDiaria"].forEach(k => { f[k] = item[k] ?? ""; });
    ["funcionario"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/cargahorarias/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["diasSemana"] ?? "CargaHoraria")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>funcionarioId</b><span>{item["funcionarioId"] ?? "—"}</span></div>
            <div className="item-field"><b>horarioEntrada</b><span>{item["horarioEntrada"] ? new Date(item["horarioEntrada"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>horarioSaida</b><span>{item["horarioSaida"] ? new Date(item["horarioSaida"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>cargaDiaria</b><span>{item["cargaDiaria"] != null ? Number(item["cargaDiaria"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>funcionario</b><span>{item.funcionarioId ? ((funcionarioList.find(o => o.id === item.funcionarioId) || {}).nome ?? ("#" + item.funcionarioId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} CargaHoraria</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">funcionarioId</label>
      <input type="number" value={form["funcionarioId"] ?? ""} onChange={e => setForm({...form, funcionarioId: e.target.value})} />
      ...
      <label className="field-label">horarioEntrada</label>
      <input type="datetime-local" value={form["horarioEntrada"] ?? ""} onChange={e => setForm({...form, horarioEntrada: e.target.value})} />
      <label className="field-label">horarioSaida</label>
      <input type="datetime-local" value={form["horarioSaida"] ?? ""} onChange={e => setForm({...form, horarioSaida: e.target.value})} />
      <label className="field-label">cargaDiaria</label>
      <input type="number" step="0.01" value={form["cargaDiaria"] ?? ""} onChange={e => setForm({...form, cargaDiaria: e.target.value})} />
      <label className="field-label">funcionario</label>
      <select value={form["funcionario"] ?? ""} onChange={e => setForm({...form, funcionario: e.target.value})}>
        <option value="">Selecione...</option>
        {(funcionarioList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Funcionario");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Ponto<span className="dot">.</span></div>
        ...
        <button className={"nav-btn" + (aba === "RegistroPonto" ? " active" : "")} onClick={() => setAba("RegistroPonto")}><span className="nav-ico">🗂️</span>RegistroPonto</button>
        <button className={"nav-btn" + (aba === "Afastamento" ? " active" : "")} onClick={() => setAba("Afastamento")}><span className="nav-ico">🗂️</span>Afastamento</button>
        <button className={"nav-btn" + (aba === "CargaHoraria" ? " active" : "")} onClick={() => setAba("CargaHoraria")}><span className="nav-ico">🗂️</span>CargaHoraria</button>
      ...
      {aba === "RegistroPonto" && <PainelRegistroPonto token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Afastamento" && <PainelAfastamento token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "CargaHoraria" && <PainelCargaHoraria token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
